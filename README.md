...
### `states.py`
Not a runnable graph, but a reference file exploring the three ways LangGraph state can be defined: `TypedDict`, `Pydantic BaseModel` (with a field validator), and Python `dataclass`.
...
`conditional_RAG.py` and `app.py` expect `academics_handbook.pdf` and `fee_structure.pdf` in the working directory, so supply your own PDFs with those names to run them.